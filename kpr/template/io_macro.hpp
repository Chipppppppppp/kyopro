...
namespace kpr::helper
{
    ...
    {
        return std::tuple<std::decay_t<decltype(static_cast<void>(idx), std::declval<std::invoke_result_t<F>>())>...>{(static_cast<void>(idx), f())...};
    }
...
#define read(type_or_init, ...) \
    auto [__VA_ARGS__] = (kpr::helper::read_impl(([&]() { \
        ...
        T res = std::move(*p); \
        p->~T(); \
        return res; }), std::make_index_sequence<kpr::helper::va_args_size(#__VA_ARGS__)>()))
