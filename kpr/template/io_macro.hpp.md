...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 2 \"kpr/template/io_macro.hpp\"\n#include <unistd.h>\n#include\
    \ <cstddef>\n#include <istream>\n#include <string_view>\n#include <tuple>\n#include\
    ...
    \ std::tuple<std::decay_t<decltype(static_cast<void>(idx), std::declval<std::invoke_result_t<F>>())>...>{(static_cast<void>(idx),\
  ...
  code: "#pragma once\n#include <unistd.h>\n#include <cstddef>\n#include <istream>\n\
    ...
    \ f, std::index_sequence<idx...>) {\n        return std::tuple<std::decay_t<decltype(static_cast<void>(idx),\
    \ std::declval<std::invoke_result_t<F>>())>...>{(static_cast<void>(idx), f())...};\n\
