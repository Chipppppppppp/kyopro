...
namespace kpr
{
    ...
    [[maybe_unused]] inline constexpr struct
    {
        ...
        constexpr KYOPRO_BASE_INT operator()(T x) const noexcept
        {
            ...
            if constexpr (digits <= std::numeric_limits<unsigned int>::digits)
    ...
    [[maybe_unused]] inline constexpr struct
    {
        ...
        constexpr KYOPRO_BASE_INT operator()(T x) const noexcept
        {
            ...
            if (x == 0)
                return 0;
            if constexpr (digits <= std::numeric_limits<unsigned int>::digits)
                return __builtin_clz(x) + digits - std::numeric_limits<unsigned int>::digits;
            else if constexpr (digits <= std::numeric_limits<unsigned long>::digits)
                return __builtin_clzl(x) + digits - std::numeric_limits<unsigned long>::digits;
            else
                return __builtin_clzll(x) + digits - std::numeric_limits<unsigned long long>::digits;
        }
    } lzero_count;
    ...
    [[maybe_unused]] inline constexpr struct
    {
        ...
        constexpr KYOPRO_BASE_INT operator()(T x) const noexcept
        {
            ...
            if (x == 0)
                return 0;
            if constexpr (digits <= std::numeric_limits<unsigned int>::digits)
                return std::numeric_limits<unsigned int>::digits - __builtin_clz(x);
            else if constexpr (digits <= std::numeric_limits<unsigned long>::digits)
