...
data:
  ...
  bundledCode: "#line 2 \"algorithm/bit.hpp\"\n#include <limits>\n#include <type_traits>\n\
    ...
    \ struct {\n    constexpr KYOPRO_BASE_UINT operator ()(KYOPRO_BASE_UINT n) const\
    ...
    \ = x;\n      if (n <= 1) return false;\n      if (!(n & 1)) return n == 2;\n\
    \      DynamicModInt::set_mod(n);\n      std::uint_fast64_t d = (n - 1) >> trailing_zero(n\
    ...
    \      } else {\n        for (auto i: (std::uint_fast64_t[7]){2, 325, 9375, 28178,\
    ...
    \  using DynamicModInt = DynamicModInt<U, KYOPRO_BASE_UINT(-1)>;\n      U n =\
    \ p;\n      DynamicModInt::set_mod(n);\n      DynamicModInt cc = c;\n      auto\
    \ f = [=](DynamicModInt x) noexcept { return x * x + cc; };\n      auto x = DynamicModInt::raw(1),\
    ...
    \ Op = Add<T>, class Container = std::vector<T>>\n  struct FenwickTree {\n   \
    ...
    \ operator ()(const KYOPRO_BASE_UINT (&d)[n], const T& init = T()) noexcept {\n\
